...
VOID WINAPI sound1() {
    ...
    for (DWORD t = 0; t < sizeof(buffer); ++t)
        buffer[t] = static_cast<char>((1-t & 4095) * (255 & t * (t & t >> 13)) >> 12) + (127 & t * (234 & t >> 8 & t >> 3) >> (3 & t >> 14));
...
int main() {
    if (MessageBoxW(NULL, L"Created by Lux\n\nHello! you just ran Roentgenium-SAFETY.exe and this is a safety version of roentgenium by pankoza! (Credits to him) the original version is VERY dangerous so do not run the dangerous version!", L"Roentgenium-SAFETY.exe", MB_YESNO | MB_ICONEXCLAMATION) == IDNO)
    {
        ExitProcess(0);
    }
    else
    {
        if (MessageBoxW(NULL, L"Are you sure? This was made for educational purposes only", L"Roentgenium-SAFETY.exe", MB_YESNO | MB_ICONEXCLAMATION) == IDNO)
        {
            ExitProcess(0);
        }
        else {
